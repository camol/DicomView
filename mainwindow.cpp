    ...
    dicom_file(0)
{
    ...
    QObject::connect(ui->actionAbout_DicomView,SIGNAL(triggered()),this,SLOT(About()));
...
{
    QMessageBox::about(this,"About Box","DicomView v1.0\n\n\n OSM project made by C@mol and Loskov.");
}

void MainWindow::Help()
{
    QMessageBox::about(this,"Help Box","DicomView Help:\n\n   1. \"File\": Has two options which opens a single DICOM file or opens the folder with many DICOM file at the same time\n\n 2. \"Image\": Menu with picture options: zooming, rotating. You can choose one of many built-in presets for displaying the image depending on the body part. \"Copy to Clipboard\" gives you a possibility to export the image to the outside world. \n\n 3. \"Cursor\": Gives you the possibilty to choose between Drag and Manual Windowing the picture with cursor mouse.\n\n 4. \"Tag Tab\": Displays the tags and their values, which were originally implemented in the DICOM file. ");
}
...
void MainWindow::Open()
{
    ...
    if (fname != "")
    {
        ...
        //DRZEWO
        /*QtreeViewItem *all_patients = new QtreeViewItem(ui->treeView);
        all_patients->setText(0, tr("All Patients"));

        QtreeViewItem *name = new QtreeViewItem(all_patients);
        name->setText(0, QString::number(dicom_file->fhight_bit(), 10)); */

        /*
        QtreeViewItem *modality = new QtreeViewItem(name);
        modality->setText(0, tr("modality"));

        QtreeViewItem *series_desc = new QtreeViewItem(modality);
...
{
    //--OTWIERANIE FOLDEROWE--
    model = new QDirModel(ui->treeView);
    ui->treeView->setModel(model);
    QString path(QFileDialog::getExistingDirectory());
    ui->treeView->setRootIndex(model->index(path));
    ui->treeView->setAnimated(true);        // animowane rozwijanie
    ui->treeView->setIndentation(10);       // odleglosc od linii glownej
    ui->treeView->setSortingEnabled(true);  //naglowki pozwalaja sortowac
}
